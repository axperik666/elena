...
function buildLayoutRequests(sheetId, colCount) {
  ...
  const requests = [
    ...
    {
      setDataValidation: {
        range: colRange(sheetId, COL.CAPI, COL.CAPI + 1, 1, MAX_ROWS),
        rule: {
          condition: {
            type: 'ONE_OF_LIST',
            values: CAPI_STATUSES.map((s) => ({ userEnteredValue: s }))
          },
          showCustomUi: true,
          strict: false,
          inputMessage: 'CAPI — статус для Facebook Conversions API'
        }
      }
    },
    {
  ...
  var statusRules = [
    ...
    { status: 'не квал', bg: { red: 1, green: 0.88, blue: 0.75 } },
    { status: 'трэш', bg: { red: 0.88, green: 0.88, blue: 0.88 } },
    { status: 'дубль', bg: { red: 0.98, green: 0.8, blue: 0.8 } },
    { status: 'отправлен', bg: { red: 0.91, green: 0.83, blue: 0.97 } },
    { status: 'ошибка', bg: { red: 1, green: 0.75, blue: 0.75 } }
  ];
  ...
  statusRules.forEach((item, idx) => {
    requests.push({
      addConditionalFormatRule: {
        rule: {
          ranges: [colRange(sheetId, COL.CAPI, COL.CAPI + 1, 1, MAX_ROWS)],
          booleanRule: {
...
async function migrateSapiHeader(spreadsheetId, tabName, sheetId) {
  const sheets = getSheetsClient();
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: tabName + '!B1',
    valueInputOption: 'RAW',
    requestBody: { values: [['CAPI']] }
  });
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId,
    requestBody: {
...
async function ensureSheetReady(spreadsheetId, tabName) {
  ...
  const capiHeader = header[COL.CAPI];

  if (header.length === HEADERS.length && capiHeader === 'SAPI') {
    await migrateSapiHeader(spreadsheetId, tabName, sheetId);
  } else if (header.length !== HEADERS.length || capiHeader !== 'CAPI') {
...
async function appendLead(spreadsheetId, tabName, data) {
  ...
  if (data.phone && newRowNum) {
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: tabName + '!E' + newRowNum,
      valueInputOption: 'RAW',
      requestBody: { values: [[compactPhoneForSheet(data.phone)]] }
    });
  }
